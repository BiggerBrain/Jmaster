...
public class _593 {
    public static class Solution1 {
        ...
        private List<List<int[]>> backTracking(List<List<int[]>> result, List<int[]> input,
                                               int pos) {
            if (pos == input.size()) {
                return result;
            }
        ...
        private boolean isValid(List<int[]> points) {
            ...
            return distance == (Math.pow(p2[0] - p3[0], 2) + Math.pow(p2[1] - p3[1], 2))
                   ...
                   && isRightAngle(p1, p2, p3) && noDuplicate(p1, p2, p3, p4);
        }

        public boolean noDuplicate(int[] p1, int[] p2, int[] p3, int[] p4) {
            return !Arrays.equals(p1, p2) && !Arrays.equals(p1, p3) && !Arrays.equals(p1, p4)
